...
def list_txt_files_from_gcs(
    bucket_name: str,
    company_id: str,
    run_folder: str
) -> List[str]:
    """
    List all TXT files for a company from GCS.
    
    Args:
        bucket_name: GCS bucket name
        company_id: Company ID
        run_folder: Run folder (e.g., 'initial_pull')
        
    Returns:
        List[str]: List of GCS paths to TXT files
    """
    try:
        client = get_gcs_client()
        bucket = client.bucket(bucket_name)
        
        prefix = f"raw/{company_id}/{run_folder}/"
        ...
        for blob in blobs:
            if blob.name.endswith('.txt'):
                # Extract base filename (without _clean.txt suffix)
                base_name = blob.name
                if blob.name.endswith('_clean.txt'):
                    base_name = blob.name[:-10] + '.txt'
                
                # Prefer _clean.txt files, but include regular .txt if no _clean version exists
                if blob.name.endswith('_clean.txt'):
                    txt_files.append(blob.name)
                    seen_base_names.add(base_name)
                elif base_name not in seen_base_names:
                    # Only add regular .txt if we haven't seen a _clean version
                    txt_files.append(blob.name)
        
        logger.info(f"Found {len(txt_files)} TXT files for {company_id}/{run_folder}")
        return txt_files
    except Exception as e:
        logger.error(f"Failed to list TXT files from GCS: {e}")
        return []


def download_text_from_gcs(
    bucket_name: str,
    gcs_path: str
) -> Optional[str]:
    """
    Download text content from GCS blob.
    ...
    Returns:
        str: Text content of the file, or None if failed
    """
    try:
        client = get_gcs_client()
        bucket = client.bucket(bucket_name)
        ...
        content = blob.download_as_text()
        logger.info(f"Downloaded text from gs://{bucket_name}/{gcs_path} ({len(content)} chars)")
        return content
    except NotFound:
        logger.warning(f"File not found in GCS: gs://{bucket_name}/{gcs_path}")
        return None
