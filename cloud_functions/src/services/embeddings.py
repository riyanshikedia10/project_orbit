...
class Embeddings:
    def __init__(self, max_retries: int = 5, base_delay: float = 0.2, max_delay: float = 30.0):
        # Initialize client here - explicitly exclude proxy settings
        # Cloud Functions may have HTTP_PROXY/HTTPS_PROXY env vars that cause issues
        if not OPENAI_API_KEY:
            raise ValueError("OPENAI_API_KEY is not set")
        
        # Temporarily remove proxy env vars if they exist to avoid conflicts
        proxy_vars = {}
        ...
        try:
            ...
            # Restore proxy vars if they were removed
...
class PineconeStorage:
    ...
    def __init__(self):
        ...
        # Initialize Pinecone client - handle proxy env vars that Cloud Functions might set
        # Temporarily remove proxy env vars if they exist to avoid conflicts
        proxy_vars = {}
        for key in ['HTTP_PROXY', 'HTTPS_PROXY', 'http_proxy', 'https_proxy']:
            if key in os.environ:
