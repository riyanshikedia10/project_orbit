#!/bin/bash
# Script to run scrape_and_index function in batches of 3 companies
...
echo "=========================================="
echo "Batch Scrape and Index Execution"
echo "=========================================="
...
echo "Function URL: $SCRAPE_AND_INDEX_URL"
echo ""
...
# Count companies using Python (more reliable than jq which might not be installed)
...
for ((batch=0; batch<$NUM_BATCHES; batch++)); do
    ...
    response=$(curl -s -w "\n%{http_code}" -X POST \
        "${SCRAPE_AND_INDEX_URL}?start=${start}&end=${end}&batch_size=${BATCH_SIZE}" \
        --max-time 600)
...
echo "  gs://${BUCKET_NAME}/scraping_results/scraping_and_indexing_results_batch_*.json"
echo ""
echo "View all results:"
echo "  gsutil ls gs://${BUCKET_NAME}/scraping_results/"
echo "=========================================="
